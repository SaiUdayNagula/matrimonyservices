...
router.post('/', (req, res) => {
  ...
  const { profileid , interest_id , description , account_id } = req.body;

  const callProcedure = `CALL usp_profile_interest_create(?, ?, ?, ?)`;
  
  db.query(callProcedure, [ profileid , interest_id , description , account_id ], (err, result) => {
    if (err) {
    ...
    if (!result[0] || !result[0][0]) {
      res.status(200).json({ message: 'Interest Added to Profile successfully.' });
    } else {
